...
**原因**：`docker-compose.yml` 中的 subnet（如 `172.20.0.0/16`）被其他 Docker 网络占用。
...
首次 `docker compose up --build` 需要下载 Go 依赖和 npm 包，通常需要 5-10 分钟。构建层已缓存，后续构建很快。
...
**症状**：`npm run serve` 后 API 请求返回 502/504。
...
参考 [docs/n9e-integration-review.md](../docs/n9e-integration-review.md) 中的 F1-F5 bug 修复记录：
- F1: SQL schema 不匹配
- F2: Token 脱敏覆盖
- F3: CmdbHostVo 缺少字段
